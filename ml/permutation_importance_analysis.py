...
def stratified_sample(X: pd.DataFrame, y: pd.Series, max_samples: int) -> tuple[pd.DataFrame, pd.Series]:
...
def compute_permutation(
    ...
    X_perm, y_perm = stratified_sample(X_imputed, y, max_samples)
    print(
...
def analyze_race(
    ...
    for race_label, subset in df.groupby("Race_Label"):
        ...
        compute_permutation(subset, target, features, race_label, max_samples)
        ...
        for include_age in [True, False]:
            ...
            for target in targets:
                ...
                compute_permutation(
                    ...
                    features,
                    label,
                    max_samples=MAX_SAMPLES_OVERALL,
                )
